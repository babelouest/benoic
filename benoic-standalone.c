 ...
 *
 * Functions definition for standalone program
 *
...
#define BENOIC_RUNNING  0
#define BENOIC_STOP     1
#define BENOIC_ERROR    2

#define BENOIC_LOG_NAME "Benoic"

// Configuration management
#define BENOIC_DEFAULT_PREFIX "benoic"
#define BENOIC_DEFAULT_PORT   2642
...
void exit_handler(int signal) {
  ...
  global_handler_variable = BENOIC_STOP;
}
...
int build_config_from_file(struct config_elements * config) {
  ...
  const char * cur_prefix, * cur_log_mode, * cur_log_level, * cur_log_file = NULL, * one_log_mode, * modules_path, 
    ...
    if (config_setting_lookup_string(database, "type", &db_type) == CONFIG_TRUE) {
      if (0 == strncmp(db_type, "sqlite3", strlen("sqlite3"))) {
        if (config_setting_lookup_string(database, "path", &db_sqlite_path) == CONFIG_TRUE) {
          config->b_config->conn = h_connect_sqlite(db_sqlite_path);
          if (config->b_config->conn == NULL) {
            config_destroy(&cfg);
        ...
        config->b_config->conn = h_connect_mariadb(db_mariadb_host, db_mariadb_user, db_mariadb_password, db_mariadb_dbname, db_mariadb_port, NULL);
        if (config->b_config->conn == NULL) {
...
int main(int argc, char ** argv) {
  ...
  global_handler_variable = BENOIC_RUNNING;
  ...
  // Initialize benoic webservice
  if (init_benoic(config->instance, config->url_prefix, config->b_config) != B_OK) {
    fprintf(stderr, "Error initializing benoic webservice\n");
    exit_server(&config, BENOIC_ERROR);
  }
